...
export class studController {
        ...
        async function listOneStud() {
            ...
            for (let i = 0; i < result[0].studentlist.length; i++) {
                ...
                const mailOptions = {
                    ...
                    html: "Go to this Url:"+'file:///home/am-n6/Rekha/online-Exam-Db/angular/index.html#!/login'+"<br>Your Password:"+encrypt+
    ...
    login(@Body() record:any , @Res() response:any){
        ...
        async function login(){
          var result=await stud.find({'studentlist.email':record.email},{'studentlist.pass':record.pass});
          console.log(result);
          if(result){
            let payload = { subject: record.pass };
